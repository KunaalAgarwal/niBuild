...
import WorkflowManagerWelcome from './WorkflowManagerWelcome.jsx';
...
/**
 * Workflow name display that flips to an inline text input on double-click.
 * Commits on Enter or blur, reverts on Escape. Empty/whitespace-only names
 * are rejected (the existing name is kept).
 */
function EditableName({ wfId, name, onRename }) {
    const [editing, setEditing] = useState(false);
    const [draft, setDraft] = useState(name);
    const inputRef = useRef(null);

    // Focus and select the whole name when entering edit mode.
    useEffect(() => {
    ...
    return (
        <span className="wm-row-name" title="Double-click to rename" onDoubleClick={startEdit}>
            {name}
        </span>
    );
}
...
function WorkflowManagerPage({
    ...
    return (
        <div className="workflow-manager-page">
            {sorted.length === 0 ? (
                <WorkflowManagerWelcome onNewWorkflow={onNewWorkflow} onImportWorkflow={onImportWorkflow} />
            ) : (
                <>
                                    ...
                                    <li key={wf.id} className="wm-row" data-wf-row={wf.id} role="row">
                                        ...
                                        <div className="wm-cell wm-cell-name">
                                            <div className="wm-row-name-line">
                                                <EditableName wfId={wf.id} name={wf.name} onRename={onRenameWorkflow} />
                                                {openWs && (
                                                    <span
